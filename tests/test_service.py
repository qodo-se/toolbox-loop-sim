...
def setup_db(path=":memory:"):
    ...
    # ensure_schema owns users/orders; audit is created here too so the tests
    # run against a database that predates the schema helper owning it.
...
def test_login():
    # test_login_{correct,wrong}_password and test_login_unknown_user cover the
    # same cases individually above; this keeps them asserted in one request path.
    c = setup_db()
